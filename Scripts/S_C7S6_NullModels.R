...
# Remarks: Last successful run: 09/12/2019; Last changes: 03/02/2020
###############Section 2: Packages########################################
...
##############Part 2: Null model performance (16 days)####################
...
df.perf.e <- data.frame(matrix(nrow = length(mf.list), ncol = 7))

###############Section 2: Modelling#######################################
for (i in c(1:length(mf.list))){
  message(paste0('Final model for ', mf.list[i]))
  df.train.perm <- df.train <- lst.all.train[[i]]
  df.test <- lst.all.test[[i]]
  df.perm <- as.data.frame(matrix(nrow = n.perm, ncol = 14))
  for (j in c(1:nrow(df.perm))){
    set.seed(j)
    df.train.perm$Macrophyte <- df.train$Macrophyte[sample(c(1:nrow(df.train)), 
                                                           size = nrow(df.train), 
                                                           replace = F)]
    lst.temp <- f.CForest(df.train.perm, cols = c(5:ncol(df.train.perm)), 
                          eval = df.test, n.ntree = 200, n.rep = 10)
    df.perm[j, ] <- f.PerfLongWide(lst.temp[[5]], mean = 2, sd = 3)
    if(j == 1){ 
      names(df.perm) <- names(f.PerfLongWide(lst.temp[[5]], mean = 2, sd = 3))
    }
  }
  # Store results
  df.info.e[i, ] <- c(mf.list[i], 'Testing', nrow(df.test), ncol(df.test))
  df.perf.e[i, ] <- apply(df.perm[, c(1:7)], 2, 
                          function(x) quantile(x, 0.95))
  if(i == 1){ names(df.perf.e) <- names(df.perm)[c(1:7)] }
}
df.perm.sum <- cbind.data.frame(df.info.e, df.perf.e)

###############Section 3: Plotting and saving#############################
if(b.save){
  write.table(df.perm.sum, paste0(s.data, 'D_C7S6_NullModel95Quantile.txt'), 
              row.names = F, sep = ',')
}

###############Section 4: Variable removal################################
rm(df.info.e, df.perf.e, i, df.train, df.test, lst.temp)
